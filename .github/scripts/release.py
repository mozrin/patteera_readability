...
def is_safe_to_reset():
    # Check if the current working directory or git root is 'github_scaffolding' or 'patteera_readability'
    # This is a safety measure to prevent accidental resets in other repos
    cwd = os.getcwd()
    return os.path.basename(cwd) in ['github_scaffolding', 'patteera_readability']
...
def main():
    ...
    if args.type == 'reset':
        if not is_safe_to_reset():
            print("Error: Reset command is only allowed in the 'github_scaffolding' or 'patteera_readability' repository.")
            sys.exit(1)
